"use client"
// Indicates that this component should only execute on the client side

import { useState } from "react"; // Import the useState hook from React for state management
import { Button, ButtonProps } from "./ui/button";
import { Copy, CopyCheck, CopyX } from "lucide-react";

// Define possible states for the copy operation
...
export function CopyEventButton({ eventId, clerkUserId, ...buttonProps }: 
    ...
    return <Button {...buttonProps } onClick={() => {
        navigator.clipboard.writeText(`${location.origin}/book/${clerkUserId}/${eventId}`
            ...
            setCopyState("copied")
            setTimeout(() => setCopyState("idle"), 2000) // Reset state after 2 seconds
        }).catch(() => {
            setCopyState("error")
